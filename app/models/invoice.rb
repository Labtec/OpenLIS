...
class Invoice
  ...
  def csv
    ...
    @claims.each do |claim|
      csv << %Q{"Lab Tests: #{claim.cpt_codes.join(', ')}. Insured ID: #{claim.insured_name.policy_number}. Our Reference #{claim.external_number}. Your Reference: #{claim.number}", 1.00, #{claim.total_price}, #{claim.total_price * (1 - DISCOUNT)},0.00,0.00,0.00,85,8510,}
    end
