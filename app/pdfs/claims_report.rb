...
class ClaimsReport < Prawn::Document
  ...
  def initialize(claims, view_context)
    ...
    letter_price_width = bounds.width - (letter_line_number_width + letter_name_width + letter_policy_number_width + letter_service_date_width + letter_claim_external_number_width + letter_claim_number_width) # was 80
    ...
    # Generate Form UB-92
    create_stamp("ub92") {
      translate(bounds.left - 35, bounds.bottom - 35) do
        ub92
      end
    }
    ...
    @claims.each do |claim|
      ...
      bounding_box([left_margin, (top_margin + field_height * 4) + 2], :width => provider_info_width, :height => field_height * 4) do
        pad(form_padding * 3) do
          indent(form_indenting) do
            text "<b>Labtec S.A.</b>", :size => 12, :inline_format => true
            text "Villa Lucre, Consultorios San Judas Tadeo #107\nTel: 222-9200 ext. 1107 • Fax: 277-7832\nmasterlab@labtecsa.com"
          end
        end
      end
      # FL 5 - Federal Tax Number
      # Required. The format is NN-NNNNNNN.
      bounding_box([left_margin + provider_info_width, (top_margin + field_height) - 1], :width => federal_tax_number_width, :height => field_height) do
        pad(form_padding) do
          text "299497-1-409892 (13)", :align => :center
        end
      end
      ...
      bounding_box([left_margin + pre_control_no_width, (top_margin + field_height * 3) - 1], :width => patient_control_number, :height => field_height) do
        pad(form_padding) do
      ...
      bounding_box([left_margin, top_margin], :width => name_width, :height => double_field_height) do
        pad(10) do
          indent(form_padding) do
            text claim.accession.patient.name_last_comma_first_mi, :size => 12
          end
        end
      end
      ...
      bounding_box([left_margin, (top_margin - field_height * 3) - 1], :width => birthdate_width, :height => field_height) do
        pad(form_padding) do
          indent(form_padding) do
      ...
      bounding_box([left_margin + birthdate_width, (top_margin - field_height * 3) - 1], :width => gender_width, :height => field_height) do
        pad(form_padding) do
      ...
      bounding_box([left_margin + occurrence_code_width, (top_margin - field_height * 5) - 3], :width => occurrence_date_width, :height => field_height) do
        pad(form_padding) do
      ...
      bounding_box([left_margin, (top_margin - field_height * 7) - 1], :width => insurance_provider_width, :height => field_height * 5) do
        pad_top(10) do
          pad(form_padding) do
            text claim.insurance_provider.name, :align => :center, :size => 16
          end
          pad(form_padding) do
            text "No. #{claim.external_number}", :align => :center, :size => 14
          end
        end
      end
      ...
      bounding_box([left_margin + prior_payments_width, (top_margin - field_height * 37) - 2], :width => due_from_patient_width, :height => field_height) do
        @claim_total_price << @total_price.sum
        pad(form_padding) do
          text "<b>#{@view.number_to_currency @total_price.sum, :unit => '', :separator => ' '}</b>", :align => :right, :inline_format => true, :size => 9
        end
      end
      # Due From Patient
      #bounding_box([left_margin + prior_payments_width, (top_margin - field_height * 40) - 2], :width => due_from_patient_width, :height => field_height) do
      #  @claim_total_price << @total_price.sum
      #  pad(form_padding) do
      #    text "#{@view.number_to_currency @total_price.sum, :unit => '', :separator => ' '}", :align => :right, :style => :bold
      #  end
      #end

      # FL 50A, B, and C - Payer Identification
      bounding_box([left_margin, (top_margin - field_height * 37) - 2], :width => due_from_patient_width, :height => field_height) do
        pad(form_padding) do
          indent(form_indenting) do
      ...
      bounding_box([left_margin, (top_margin - field_height * 42) - 5], :width => insured_name_width, :height => field_height) do
        pad(form_padding) do
          indent(form_indenting) do
      ...
      bounding_box([left_margin + insured_name_width + patient_relationship_width, (top_margin - field_height * 42) - 5], :width => insured_unique_id_width, :height => field_height) do
        pad(form_padding) do
      ...
      diag_codes.each_with_index do |diag_code, i|
        bounding_box([left_margin + code_width * i, (top_margin - field_height * 50) - 2], :width => code_width, :height => field_height) do
          pad(form_padding) do
            text diag_code, :align => :center
          end
        end
      end
      # Next page table
      if (panels_table.size + lab_tests_table.size) > main_table_rows
        lab_tests_table.insert((main_table_rows - 1 - panels_table.size), ['', '<b><i>PAGE 1 OF 2</i></b>', '', '', '', ''])
        lab_tests_table.insert((lab_tests_table.size), ['', '<b><i>PAGE 2 OF 2</i></b>', '', '', '', ''])
      end
      ...
      bounding_box([left_margin, (top_margin - field_height * 13) - 1], :width => claims_table_width, :height => field_height * main_table_rows) do
        # Output Panels Table
    ...
    @claims.each_with_index.map do |claim, index|
      claims_table += [[
        ...
        claim.number,
        @view.number_to_currency(@claim_total_price[index], :unit => '')
      ]]
    end
    claims_table += [["", "", "", "", "", "<b>Gran Total:</b>", "<b>#{@view.number_to_currency(@claim_total_price.sum, :unit => 'B/.')}</b>"]]
    ...
    table claims_table,
      ...
      :cell_style => { :padding => [table_padding, table_padding], :inline_format => true } do |t|
        ...
        t.row(0).border_top_color = colors[:black]
        t.row(0).border_top_width = 1
        t.row(0).border_bottom_color = colors[:black]
        t.row(0).border_bottom_width = 0.5
        t.row(0).borders = [:top, :bottom]
        t.row(-1).border_top_color = colors[:black]
        t.row(-1).border_top_width = 0.5
        t.row(-1).border_bottom_color = colors[:black]
