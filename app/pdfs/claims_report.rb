    ...
    @claims.each do |claim|
      ...
      panels_table = claim.accession.panels.with_price.map do |panel|
        ...
        [
          ...
          '1',
          "#{@view.number_with_precision panel.prices.find_by_price_list_id(1).amount, precision: 2, separator: ' ' if panel.prices.find_by_price_list_id(1)}"
        ]
      end
      # Lab Tests Table
      @lab_tests = []
      ...
      lab_tests_table = @lab_tests.map do |lab_test|
        ...
        [
          ...
          claim.accession.drawn_at.to_date.to_formatted_s(:mmddyy),
          '1',
          "#{@view.number_with_precision lab_test.prices.find_by_price_list_id(1).amount, precision: 2, separator: ' ' if lab_test.prices.find_by_price_list_id(1)}"
        ]
      end
      # TOTALS
      ...
      bounding_box([left_margin + prior_payments_width, (top_margin - field_height * 37)], width: due_from_patient_width, height: field_height) do
        pad(form_padding) do
          text "#{@view.number_with_precision @total_price.sum, precision: 2, separator: ' '}", align: :right
        end
      end
