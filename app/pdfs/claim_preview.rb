class ClaimPreview < Prawn::Document
  ...
  def initialize(claim, view_context)
    ...
    bounding_box([left_margin + provider_info_width, (top_margin + field_height) - 1], :width => federal_tax_number_width, :height => field_height) do
      pad(form_padding) do
        text "299497-1-409892", :align => :center
      end
    end
    ...
    bounding_box([left_margin, (top_margin - field_height * 13) - 1], :width => claims_table_width, :height => field_height * 23) do
      @total_price = []
      # Panels Table
