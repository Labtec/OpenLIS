    ...
    @results.each do |department, test_results|
      ...
      test_results.each do |result|
        if result.flag.present?
          ...
          cell_col1 = make_cell content: format_value(result).gsub(/</, '&lt; ').gsub(/&lt; i/, '<i').gsub(/&lt; s/, '<s').gsub(/&lt; \//, '</'), background_color: REPORT_COLORS[:highlight_gray], inline_format: true
        else
          cell_col0 = make_cell content: result.lab_test_name, inline_format: true
          cell_col1 = make_cell content: format_value(result).gsub(/</, '&lt; ').gsub(/&lt; i/, '<i').gsub(/&lt; s/, '<s').gsub(/&lt; \//, '</'), inline_format: true
        end
