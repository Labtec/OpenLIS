...
class DiscountValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    ...
    record.errors.add(attribute, options[:message] || I18n.t("errors.messages.must_be_between_0_and_100")) if record.discount_percentage? && !value.in?(0..100)
    return unless record.discount_currency? && value > record.list_price
