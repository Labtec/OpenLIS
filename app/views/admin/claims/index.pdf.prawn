...
require "prawn"
require "#{File.dirname(__FILE__)}/ub92"
require "#{RAILS_ROOT}/app/views/shared/logo_labtec"
...
# Generate Form UB-92
pdf.create_stamp("ub92") {
  pdf.translate(pdf.bounds.left - 35, pdf.bounds.bottom - 35) do
    pdf.ub92
  end
}
...
@claims.each do |claim|
  ...
  pdf.bounding_box([left_margin + provider_info_width, (top_margin + field_height) - 1], :width => federal_tax_number_width, :height => field_height) do
    pdf.pad(form_padding) do
      pdf.text "299497-1-409892 (13)", :align => :center
    end
  end  
  ...
  #  @claim_total_price << @total_price.compact.sum
  #  pdf.pad(form_padding) do
  #    pdf.text "#{number_to_currency @total_price.copact.sum, :unit => '', :separator => ' '}", :align => :right, :style => :bold
  #  end
  #end
  ...
  pdf.bounding_box([left_margin, (top_margin - field_height * 13) - 1], :width => claims_table_width, :height => field_height * main_table_rows) do
    # Output Panels Table
...
# Corporate logo
pdf.translate(pdf.bounds.left, pdf.bounds.top - 50) do
  pdf.logo_labtec
end

pdf.font_size = 9

pdf.fill_color colors[:black] # Otherwise takes the color of the logo
pdf.draw_text "R.U.C. 299497-1-409892 D.V. 13", :at => [60, pdf.cursor - 55]

pdf.move_down 70
pdf.text l(Date.today, :format => :long), :align => :right

pdf.move_down 50

pdf.text "<b>AXA — Panamá</b>", :align => :center, :inline_format => true
pdf.text "<b>Entrada de Reclamos</b>", :align => :center, :inline_format => true
...
@claims.each_with_index.map do |claim, index|
  claims_table += [[
    ...
    claim.number,
    number_to_currency(@claim_total_price[index], :unit => '')
  ]]
end
claims_table += [["", "", "", "", "", "<b>Gran Total:</b>", "<b>#{number_to_currency(@claim_total_price.compact.sum, :unit => 'B/.')}</b>"]]

pdf.table claims_table,
  :header => true,
  :position => :center,
